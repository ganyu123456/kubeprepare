...
#   namespace         Kubernetes 命名空间（选填，默认: postgresql）
#   release-name      Helm release 名称（选填，默认: postgresql）
#
# 示例:
#   sudo ./install.sh harbor.example.com
#   sudo ./install.sh harbor.example.com harbor.example.com postgresql my-pg
#
# 支持两种安装模式（由脚本自动选择）:
...
HARBOR_ADDR="${1:-}"
CHART_REPO_ADDR="${2:-${1:-}}"
NAMESPACE="${3:-postgresql}"
RELEASE_NAME="${4:-postgresql}"
CHART_VERSION="18.5.14"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INSTALL_LOG="/var/log/postgresql-install.log"
...
if [ -z "$HARBOR_ADDR" ]; then
  ...
  echo ""
  echo "示例: $0 harbor.example.com harbor.example.com postgresql postgresql"
  exit 1
fi
...
# 检查 chart 文件
CHART_TGZ=$(ls "$SCRIPT_DIR/charts/${CHART_NAME:-postgresql}-${CHART_VERSION}.tgz" 2>/dev/null | head -1 || \
...
if [ "$CHART_PUSH_SUCCESS" = true ]; then
  info "模式A：从 Harbor OCI 仓库安装 (oci://${CHART_REPO_ADDR}/charts/postgresql)"
  if $HELM_BIN install "$RELEASE_NAME" \
      "oci://${CHART_REPO_ADDR}/charts/postgresql" \
...
if [ "$INSTALL_SUCCESS" = false ]; then
  ...
  # 找自包含的主 chart（文件名最长的通常是主 chart，依赖 chart 较小）
  MAIN_CHART=$(ls -S "$SCRIPT_DIR/charts/postgresql-"*.tgz 2>/dev/null | head -1)
  ...
  if $HELM_BIN install "$RELEASE_NAME" "$MAIN_CHART" \
    ...
    fail "PostgreSQL 安装失败，请检查以上错误日志"
  fi
fi
...
if [ -n "$KUBECTL_BIN" ]; then
  ...
  log ""
  log "获取 postgres 密码:"
  log "  ${KUBECTL_BIN} get secret -n ${NAMESPACE} ${RELEASE_NAME}-postgresql \\"
  log "    -o jsonpath='{.data.postgres-password}' | base64 -d && echo"
  log ""
  log "连接到 PostgreSQL:"
  log "  ${KUBECTL_BIN} run -it --rm pgcli --image=${HARBOR_ADDR}/library/postgresql:* \\"
  log "    --env PGPASSWORD=<password> -- psql -h ${RELEASE_NAME}-postgresql.${NAMESPACE}.svc -U postgres"
fi
log ""
log "卸载:"
