...
# Redis 卸载脚本
# ────────────────────────────────────────────────────────────
# 用途: 通过 Helm 卸载 Redis，并可选清理 PVC 和命名空间
#
# 用法:
#   ./cleanup.sh [namespace] [release-name] [--delete-pvc] [--delete-namespace]
#
# 参数:
#   namespace         命名空间（默认: redis）
#   release-name      Helm release 名称（默认: redis）
#   --delete-pvc      同时删除 PersistentVolumeClaim（数据将丢失！）
#   --delete-namespace 同时删除命名空间
#
# 示例:
#   ./cleanup.sh
#   ./cleanup.sh redis my-redis
#   ./cleanup.sh redis my-redis --delete-pvc --delete-namespace
# ────────────────────────────────────────────────────────────

NAMESPACE="${1:-redis}"
RELEASE_NAME="${2:-redis}"
DELETE_PVC=false
